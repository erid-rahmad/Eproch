...
import com.bhp.opusb.config.KafkaProperties;
...
class OpusWebAppKafkaResourceIT {
    ...
    void setup() {
        ...
        OpusWebAppKafkaResource kafkaResource = new OpusWebAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/opus-web-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/opus-web-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
