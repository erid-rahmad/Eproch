...
export default class ADTabComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('ad-tab-heading'));
...
export class ADTabDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('opusWebApp.aDTab.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-aDTab'));
}
