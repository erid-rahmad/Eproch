...
export default class ADWindowComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('ad-window-heading'));
...
export class ADWindowDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('opusWebApp.aDWindow.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-aDWindow'));
}
