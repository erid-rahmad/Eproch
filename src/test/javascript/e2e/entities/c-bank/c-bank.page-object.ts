...
export default class CBankComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('c-bank-heading'));
...
export class CBankDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('opusWebApp.cBank.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-cBank'));
}
