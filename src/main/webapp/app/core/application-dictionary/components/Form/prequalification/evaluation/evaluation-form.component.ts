...
export default class EvaluationForm extends Mixins(AccessLevelMixin, Props) {
  ...
  save(status?: string) {
    ...
    this.data.evaluationStatus = status ? status: 'DRF';
    ...
    const baseHeaderApiUrl = "/api/m-prequalification-submissions"
    const baseApiUrl = "/api/m-prequalification-evals"
    const evaluationName = 'Prequalification'
    this.commonService(baseHeaderApiUrl).update(this.data).then((res)=>{
      ...
      .catch(err => {
        console.error('Failed to save the evaluation. %O', err);
        this.$message.error(`Failed to save the ${evaluationName} evaluation.`);
      })
      .finally(() => {
        this.$emit('update:loading', false); this.loading = false;
  ...
  handleButton(){
    if (this.data.evaluationStatus==="SMT"){
      this.readOnly = true;
    }
    if (this.data.evaluationStatus==="RJC" ){
      this.readOnly = true;
      this.readOnlyApp = true;
    }
    if (this.data.evaluationStatus==="APP"){
      this.readOnly = true;
      this.readOnlyApp = true;
    }
  }
