...
export default class BiddingNegotiationLine extends mixins(AccessLevelMixin, NegotiationLineProp) {
  ...
  agreed: any[] = [{vendorName:'PT. Buana', result:'pass', negotiationStatus:'agreed'}];
  inProgress: any[] = [];
  disagreed: any[] = [{vendorName:'PT. Cakra', result:'fail', negotiationStatus:'disagreed'}];
  notStarted: any[] = [];
  ...
  viewNegotiationWindow(row:any){
    ...
    (<any>document.querySelector("#innerCard")).style.height = "fit-content";
  ...
  closeLine(){
    ...
    (<any>document.querySelector("#innerCard")).style.height = "initial";
