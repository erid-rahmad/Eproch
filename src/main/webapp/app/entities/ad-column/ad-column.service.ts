...
export default class ADColumnService {
  public find(id: number): Promise<IADColumn> {
    return new Promise<IADColumn>((resolve, reject) => {
      axios
        .get(`${baseApiUrl}/${id}`)
        .then(function(res) {
          resolve(res.data);
  ...
  public create(entity: IADColumn): Promise<IADColumn> {
    return new Promise<IADColumn>((resolve, reject) => {
      axios
  ...
  public update(entity: IADColumn): Promise<IADColumn> {
    return new Promise<IADColumn>((resolve, reject) => {
      axios
