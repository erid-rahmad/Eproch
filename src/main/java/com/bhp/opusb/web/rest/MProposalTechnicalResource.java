...
public class MProposalTechnicalResource {
    ...
    /**
     * {@code POST  /m-proposal-technicals/requirements} : Create or update mProposalTechnicals.
     *
     * @param mProposalTechnicalDTOs the list of mProposalTechnicalDTOs to create or update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the mProposalTechnicalDTOs.
     */
    @PostMapping("/m-proposal-technicals/requirements")
    public ResponseEntity<List<MProposalTechnicalDTO>> createMProposalTechnicals(@Valid @RequestBody List<MProposalTechnicalDTO> mProposalTechnicalDTOs) {
        log.debug("REST request to save MProposalTechnicals. size : {}", mProposalTechnicalDTOs.size());
        List<MProposalTechnicalDTO> result = mProposalTechnicalService.saveRequirements(mProposalTechnicalDTOs);
